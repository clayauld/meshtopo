...
def app_topic():
    ...
    app._callsign_cache = {}
    app.device_states = {}
    app.stats = {"messages_received": 0, "messages_processed": 0, "position_updates_sent": 0, "errors": 0}
    return app

def test_extract_channel_from_topic(app_topic):
    """Test channel extraction from various topic formats."""
    assert app_topic._extract_channel_from_topic("msh/US/2/json/LongFast/!12345678") == "LongFast"
    assert app_topic._extract_channel_from_topic("msh/EU_868/2/json/TeamA/!abcdef") == "TeamA"
    assert app_topic._extract_channel_from_topic("msh/region/2/json/Channel-With-Dashes/!123") == "Channel-With-Dashes"
...
async def test_routing_to_unmapped_node_on_matched_channel(app_topic):
    """Test that a node not in the 'nodes' list is still routed if the channel matches."""
    msg = {
        "from": "456",
        "type": "position",
    ...
    # Should still route to tenant_beta, using hardware_id as callsign (default fallback)
    app_topic.caltopo_reporter.send_position_update.assert_called_once_with(
