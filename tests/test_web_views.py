...
async def test_login_get(cli):
    ...
    assert (
        "password" in text.lower() or "<form" in text.lower() or "<html" in text.lower()
    )
...
async def test_login_post_valid_db_hash(mock_gateway_app):
    ...
    salt = bcrypt.gensalt()
    mock_gateway_app.web_config["admin_password_hash"] = bcrypt.hashpw(
        b"db_admin", salt
    ).decode("utf-8")
    app = await create_app(mock_gateway_app)
    server = TestServer(app)
...
async def test_config_post(cli):
    ...
    data = {
        "team_id": "new_team",
        "caltopo_connect_key": "new_key",
        "caltopo_group": "new_group",
