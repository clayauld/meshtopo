...
class TestMQTTTopicFormat(unittest.TestCase):
    ...
    def test_region_code_topic_format(self):
        """Test that MQTT topic format uses REGION placeholder."""
        # Test that config files contain REGION placeholder
        config_files = [
            ...
            if config_path.exists():
                with open(config_path, 'r') as f:
                    ...
                    self.assertIn('msh/REGION/2/json/+/+', content,
                                f"Config file {config_file} should contain REGION placeholder")
                    self.assertNotIn('msh/US/2/json/+/+', content,
                                   f"Config file {config_file} should not contain hardcoded US")
