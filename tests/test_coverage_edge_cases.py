...
def test_caltopo_reporter_url_validation():
    ...
    """
    # 1. Test Valid URL
    with patch.dict(os.environ, {"CALTOPO_API_URL": "https://caltopo.com/api/v1"}, clear=True):
        import caltopo_reporter
        reload(caltopo_reporter)
        # Should not raise

    # 2. Test Invalid URL (hostname mismatch)
    with patch.dict(os.environ, {"CALTOPO_API_URL": "https://evil-site.com/api"}, clear=True):
        # Validation is now triggered on __init__, not just import
        # So we need to instantiate it to test validation, OR call the validation function
        ...
        with pytest.raises(ValueError, match="Hostname must be 'caltopo.com'"):
            # reload(caltopo_reporter) # This might not trigger it if it's in __init__ now
