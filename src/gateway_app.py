...
class GatewayApp:
    ...
    def _process_message(self, data: Dict[str, Any]) -> None:
        ...
        try:
            # Extract numeric node ID from the 'from' field
    ...
    def _process_position_message(self, data: Dict[str, Any], numeric_node_id: str) -> None:
        ...
        if not isinstance(data, dict):
            self.logger.error("Invalid data type for position message")
            self.stats["errors"] += 1
            return
        
        if not isinstance(numeric_node_id, str):
            self.logger.error("Invalid numeric_node_id type for position message")
            self.stats["errors"] += 1
            return
        ...
        if not hardware_id:
            ...
            if sender and sender.startswith("!"):
                # Validate sender type at runtime instead of using assert
                if not isinstance(sender, str):
                    self.logger.error("Invalid sender type in position message")
                    self.stats["errors"] += 1
                    return
                hardware_id = sender
        ...
        # This check must happen early to prevent unauthorized access
