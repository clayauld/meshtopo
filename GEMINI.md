...
- **Development Environment:** It is highly recommended to use a Python virtual environment (venv) to manage project dependencies. This isolates the project's dependencies from your system's Python packages. For development, install the package in editable mode: `pip install -e '.[dev]'`. This ensures that imports resolve correctly without manipulating `sys.path` manually.
