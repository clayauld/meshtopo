    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.caltopo_reporter.test_connection():
                self.logger.warning("CalTopo API connectivity test failed, but continuing...")
    ...
    def start(self) -> None:
        ...
        try:
            ...
            while self.running:
                ...
                if self.stats['start_time'] and (time.time() - self.stats['start_time']) % 60 < 1:
    ...
    def _process_message(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if message_type != 'position':
                self.logger.debug(f"Received non-position message from {node_id}: {message_type}")
                return
            
            # Extract payload
            payload = data.get('payload')
            if not payload:
                self.logger.warning(f"Received position message from {node_id} without payload")
                return
            ...
            if latitude_i is None or longitude_i is None:
                self.logger.warning(f"Received position message from {node_id} without coordinates")
                return
            ...
            self.logger.debug(f"Processing position from {node_id}: {latitude}, {longitude}")
            
            # Send to CalTopo
            success = self.caltopo_reporter.send_position_update(node_id, latitude, longitude)
